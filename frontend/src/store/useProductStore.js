...
export const useProductStore = create((set) => ({
    ...
    fetshFeaturedProducts : async () => {
        set({loading :true});
        try {
            const response = await axiosInstanse.get("products/featured");
            set({products: response.data , loading: false});
        } catch (error) {
            set({error: "Failed to fetsh products", loading:false});
            console.log("Error fetshing featured products", error);
        }
    }
}));    